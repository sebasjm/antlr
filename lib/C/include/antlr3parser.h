...
{
    /** All implementors of this interface require a pointer to their selves,
     *  passed back to them when an API function is called.
     */
    void			* me;
     ...
     * for the parser.
     */
    void			(*setTokenStream)	(void	* parser, pANTLR3_TOKEN_STREAM);

    /** A pointer to a function that returns the token stream for this 
     *  parser.
     */
    pANTLR3_TOKEN_STREAM	(*getTokenStream)	(void	* parser);

    /** Pointer to function to reset the parser's state
     */
    void			(*reset)		(void   * parser);
